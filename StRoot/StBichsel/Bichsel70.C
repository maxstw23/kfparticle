...
Double_t bichsel70(Double_t *x,Double_t *par) {
  ...
  Double_t val1 = m_Bichsel->GetI70(TMath::Log10(poverm),1.);
  Double_t val2 = m_Bichsel->GetI70(TMath::Log10(pove),1.);
  Double_t val  = 0;
  if (val1 > 0 && val2 > 0) val = TMath::Log(val1/val2);
  //  cout << "pove\t" << pove << "\t" << val1 << "\tpoverm\t" << val2 << "\t" << val << endl;
  return val;
}
//________________________________________________________________________________

void Bichsel70(const Char_t *tag = "") {
#if 1
